...
class ReviewScope(unittest.TestCase):
    ...
    def test_failure_and_blocker_precedence_with_unassessed_criteria(self):
        raw = answer()
        code = raw["artifacts"][2]
        code["invasive"]["deficit"] = None
        code["optimal"].update(deficit=2, evidence=[
            {"artifact_id": "code", "source": "after", "quote": "replacement"}])
        self.assertEqual(changes.summarize(raw, change())["artifacts"][2]["outcome"], "fail")
        code["blockers"] = ["Synthetic blocker."]
        self.assertEqual(changes.summarize(raw, change())["artifacts"][2]["outcome"], "blocked")
